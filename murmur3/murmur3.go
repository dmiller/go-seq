...
func HashString(input string) uint32 {
	...
	for i := 3; i < len; i += 4 {
		key := input[i-3] | input[i-2] << 8 | input[i-1] << 16 | input[i] << 24
		key = mixKey(key);
		hash = mixHash(hash,key)
	}
	...
	{	
		...
		case 1:
			key = input[len-1]
		case 2:
			key = input[len-2] | input[len-1] << 8
		case 3:
			key = input[len-3] | input[len-2] << 8 | input[len-1] << 16
		}
		key = mixKey(key)
		hash = mixHash(hash,key)
	}

    return finalize(hash, len)
}
...
func finalizeCollHash(hash uint32, count int32) {
    h1 := seed
    k1 := mixkey(hash)
    h1 = MixHash(h1,k1)
...
func rotateLeft(x uint32, n int32) uint32 {
	return (x << n) | (x >> (32 - n))
